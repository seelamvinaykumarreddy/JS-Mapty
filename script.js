...
class App {
  ...
  _hideForm() {
    ...
    setTimeout(() => (form.style.display = 'gird'));
  }
  ...
  _newWorkout(e) {
    ...
    const checkpostive = (...inputs) => inputs.every(inp => inp > 0);
    e.preventDefault();
    ...
    if (type === 'running') {
      ...
      if (
        ...
        !checkpostive(distance, duration, cadence)
      )
        return alert('Input has to be postive number');
    ...
    if (type === 'cycling') {
      ...
      if (
        ...
        !checkpostive(distance, duration)
      )
        return alert('Input has to be postive number');
